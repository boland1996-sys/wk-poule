...
// ── PUNTENSYSTEEM 10/7/5/2 ────────────────────────────────────────────────
// 10pt: exacte uitslag
//  7pt: winnaar goed + goals van één team goed
//  5pt: gelijkspel goed maar niet exact
//  2pt: alleen goals van één team goed
function scorePts(ph, pa, mh, ma) {
  ph = +ph; pa = +pa; mh = +mh; ma = +ma;
  if (ph === mh && pa === ma) return { pts: 10, label: "+10 ✓", cls: "pts-10" };
  const predDraw = ph === pa, realDraw = mh === ma;
  const predWin = ph > pa, realWin = mh > ma;
  const predLoss = ph < pa, realLoss = mh < ma;
  const winnerOk = (predWin && realWin) || (predLoss && realLoss) || (predDraw && realDraw);
  // winnaar + één team goals goed (niet exact)
  if (winnerOk && !predDraw && (ph === mh || pa === ma)) return { pts: 7, label: "+7", cls: "pts-7" };
  // gelijkspel goed maar niet exact
  if (predDraw && realDraw) return { pts: 5, label: "+5", cls: "pts-5" };
  // alleen winnaar goed (geen gelijkspel)
  if (winnerOk && !predDraw) return { pts: 0, label: "0", cls: "pts-0" }; // winnaar only = 0 in dit systeem
  // één team goals goed
  if (ph === mh || pa === ma) return { pts: 2, label: "+2", cls: "pts-2" };
  return { pts: 0, label: "0", cls: "pts-0" };
}

function calcMatchPts(preds, matches) {
  let pts = 0, exact = 0, near = 0;
  for (const p of preds) {
    ...
    pts += r.pts;
    if (r.pts === 10) exact++;
    else if (r.pts >= 5) near++;
  }
  return { pts, exact, near };
}
...
// ── GROEPSFASE EINDSTANDEN PTS (5pt per juiste positie) ───────────────────
function calcStandingPts(userStanding, realStanding) {
  if (!userStanding || !realStanding) return 0;
  let pts = 0;
  for (let i = 0; i < 4; i++) {
    if (userStanding[i] && realStanding[i] && userStanding[i] === realStanding[i]) pts += 5;
  }
  return pts;
}
...
:root{
  --gr:#00c97d;--gr2:#00a868;--am:#fbbf24;--am2:#f59e0b;
  --re:#f43f5e;--bg:#080b12;--c1:#0e1320;--c2:#141b2d;--c3:#1c2540;
  --bd:#1e2d4a;--t1:#eef2ff;--t2:#8899bb;--t3:#455575;
...
.hdr{background:linear-gradient(180deg,var(--c1) 0%,rgba(14,19,32,.95) 100%);backdrop-filter:blur(12px);border-bottom:1px solid var(--bd);position:sticky;top:0;z-index:100}
...
.logo-chip{background:var(--gr);color:#000;font-size:9px;font-weight:900;border-radius:4px;padding:2px 6px;letter-spacing:.8px;text-transform:uppercase}

.bnav{position:fixed;bottom:0;left:0;right:0;background:rgba(14,19,32,.97);backdrop-filter:blur(16px);border-top:1px solid var(--bd);display:flex;z-index:100;padding-bottom:env(safe-area-inset-bottom,0)}
...
.lb-row.me{background:linear-gradient(90deg,rgba(0,201,125,.07),transparent 60%)}
...
.score-btn.done{border-color:rgba(0,201,125,.3);color:var(--gr);background:rgba(0,201,125,.05)}
...
.pill.green{background:rgba(0,201,125,.1);border-color:rgba(0,201,125,.2);color:var(--gr)}
.pts-badge{font-size:11px;font-weight:700;padding:2px 7px;border-radius:5px;white-space:nowrap}
.pts-10{background:rgba(0,201,125,.2);color:#00c97d;border:1px solid rgba(0,201,125,.3)}
.pts-7{background:rgba(59,130,246,.15);color:#60a5fa;border:1px solid rgba(59,130,246,.25)}
.pts-5{background:rgba(251,191,36,.15);color:var(--am);border:1px solid rgba(251,191,36,.25)}
.pts-2{background:rgba(168,85,247,.15);color:#c084fc;border:1px solid rgba(168,85,247,.25)}
.pts-0{background:rgba(244,63,94,.12);color:var(--re);border:1px solid rgba(244,63,94,.2)}
.pts-3{background:rgba(0,201,125,.15);color:var(--gr)}
...
.btn-green{background:var(--gr);color:#000;padding:12px 20px;font-size:14px;width:100%;border-radius:var(--r)}
...
.btn-confirm{background:var(--gr);color:#000;border:none;padding:6px 12px;font-size:12px;border-radius:7px;font-weight:700}
...
.gtab.on{background:var(--gr);border-color:var(--gr);color:#000}
...
.ptab.on{background:var(--gr);border-color:var(--gr);color:#000}
...
.bq.ok{border-color:rgba(0,201,125,.4)}
...
.auth-hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 50% 120%,rgba(0,201,125,.08),transparent 60%)}
...
function PredDist({ matchId, preds, matchDone }) {
  ...
  const losses = mp.filter(p => +p.home_goals < +p.away_goals).length;
...
// ── GROUP CARD ────────────────────────────────────────────────────────────
function GroupCard({ group, matches, isAdmin, myPreds, allPreds, onScore, onLock, onPred }) {
      ...
      {gm.map(m => {
        ...
        return (
          <div key={m.id} className="mr">
            ...
            <PredDist matchId={m.id} preds={allPreds} matchDone={done} />
            {!isAdmin && isP && (
    ...
    <div>
      <div className="infobox">
        ...
        <div className="infobox-t">
          ...
          {bonusR && !locked && <span style={{ color: "var(--gr)", fontWeight: 700 }}> Jij hebt {correct}/{total} goed!</span>}
          {bonusR && locked && <span style={{ color: "var(--gr)", fontWeight: 700 }}> Jij hebt {correct}/{total} goed!</span>}
        </div>
      </div>
      {BONUS_QS.map((q, i) => {
        ...
        return (
          <div key={q.id} className={`bq${r ? ok ? " ok" : " no" : ""}`} style={{ animationDelay: `${i * 40}ms` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 9, gap: 8 }}>
              ...
              {r && <span className={`pts-badge ${ok ? "pts-10" : "pts-0"}`} style={{ flexShrink: 0 }}>{ok ? "+5 ✓" : "0 ✗"}</span>}
            </div>
            {locked ? (
  ...
  return (
    <div className="stand-pred" style={{ marginTop: 10 }}>
      <div className="stand-pred-title">
        ...
        {pts !== null && <span style={{ color: "var(--gr)", marginLeft: 6 }}>+{pts}pt</span>}
      </div>
      {order.map((team, i) => (
        <div key={team} className="stand-pred-row">
          ...
          <div style={{ flex: 1, fontSize: 12, fontWeight: 700 }}>{team}</div>
          {!locked && (
    ...
    <div className="auth">
      ...
      <div className="auth-hero">
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ fontSize: 48, marginBottom: 12, filter: "drop-shadow(0 0 16px rgba(0,201,125,.35))" }}>🏆</div>
...
export default function App() {
  ...
  const [standingPreds, setStandingPreds] = useState([]); // [{user_id, group, order:[team,...]}]
  ...
  // Modal state
  const [modal, setModal] = useState(null); // {type:"delete"|"resetpw", user}
  ...
  const resetPw = async (uid, pw) => {
    if (pw.length < 6) { toast$("❌ Minimum 6 tekens"); return; }
    setModalLoading(true);
    const { error } = await sb.from("users").update({ pw_hash: hashPw(pw) }).eq("id", uid);
    setModalLoading(false);
    if (!error) { toast$(`🔑 Wachtwoord gewijzigd`); setModal(null); }
  ...
  const saveStandingPred = async (group, order) => {
    ...
    toast$("✓ Eindstand opgeslagen");
  };

  const calcStandingPtsForUser = (uid) => {
    let total = 0;
    for (const g of GROUPS) {
      const sp = standingPreds.find(s => s.user_id === uid && s.group === g);
      if (!sp) continue;
      const real = getStanding(matches, g);
      const allDone = GROUP_TEAMS[g].every(t => {
        const gm = matches.filter(m => m.grp === g && m.phase === "group");
        return gm.length > 0 && gm.every(m => m.home_goals != null);
      });
      if (!allDone) continue;
      total += calcStandingPts(sp.order, real.map(r => r.team));
    }
    return total;
  };
  ...
  const leaderboard = [...users].map(u => {
    ...
    const { pts, exact, near } = calcMatchPts(up, matches);
    const ba = bonusA.find(b => b.user_id === u.id)?.answers || {};
    const bp = calcBonusPts(ba, bonusR);
    const sp = calcStandingPtsForUser(u.id);
    return { ...u, pts: pts + bp + sp, mp: pts, bp, sp, exact, near, pc: up.length };
  }).sort((a, b) => b.pts - a.pts || b.exact - a.exact);

  const maxPts = leaderboard[0]?.pts || 1;

  // Next match — alle fases
  const nextMatch = matches
    .filter(m => m.home_goals == null && m.home && m.away && !m.home.startsWith("Winnaar") && !m.home.startsWith("1e") && !m.home.startsWith("2e") && !m.home.startsWith("Beste"))
    .sort((a, b) => {
  ...
  const realStanding = getStanding(matches, grp);
      ...
      <header className="hdr">
        <div className="hdr-in">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {isAdmin && <span style={{ background: "rgba(0,201,125,.15)", color: "var(--gr)", fontSize: 9, fontWeight: 900, borderRadius: 4, padding: "2px 7px", letterSpacing: .5, border: "1px solid rgba(0,201,125,.2)" }}>ADMIN</span>}
      ...
      <div className="page">
          ...
          <div className="fu">
            ...
            {countdown.started && nextMatch && (
              <div className="next-match">
                ...
                <div style={{ fontSize: 11, color: "var(--t3)", marginTop: 6 }}>📅 {nextMatch.match_date} · {nextMatch.phase === "group" ? `Groep ${nextMatch.grp}` : KO_PHASES.find(p => p.id === nextMatch.phase)?.full || nextMatch.phase}</div>
              </div>
            )}

            <div className="sec-title">Tussenstand</div>
            <div className="sec-sub">10pt exact · 7pt bijna · 5pt gelijkspel · 2pt gedeeltelijk · 5pt bonus · 5pt eindstand</div>
                    ...
                    <div key={u.id} className={`lb-row${isMe ? " me" : ""}`} style={{ animationDelay: `${i * 40}ms` }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                          ...
                          {isMe && <span style={{ background: "rgba(0,201,125,.15)", color: "var(--gr)", fontSize: 9, fontWeight: 900, borderRadius: 3, padding: "1px 5px", border: "1px solid rgba(0,201,125,.2)" }}>JIJ</span>}
                        </div>
                        ...
                        <div className="lb-sub" style={{ marginTop: 3 }}>
                          ...
                          <span>{u.near}× bijna</span>
                          {u.bp > 0 && <><span style={{ color: "var(--bd)" }}>·</span><span style={{ color: "var(--am)" }}>+{u.bp}pt bonus</span></>}
                          {u.sp > 0 && <><span style={{ color: "var(--bd)" }}>·</span><span style={{ color: "#60a5fa" }}>+{u.sp}pt stand</span></>}
                        </div>
                      </div>
        ...
        {tab === "groepen" && (
          <div className="fu">
            ...
            {!isAdmin && (
              <StandingPredict
                group={grp}
                myStandingPred={myStandingPred}
                onSave={saveStandingPred}
                wkStarted={wkStarted}
        ...
        {tab === "bonus" && (
          <div className="fu">
            ...
            <div className="sec-sub">{BONUS_QS.length} vragen · 5 punten per goed antwoord{wkStarted ? " · 🔒 Gesloten" : " · sluit bij WK-start"}</div>
        ...
        {tab === "mijn" && !isAdmin && (
          <div className="fu">
            ...
            {(() => {
              const { pts: mp, exact, near } = calcMatchPts(myPreds, matches);
              const bp = calcBonusPts(myBonusAns, bonusR);
              const sp = calcStandingPtsForUser(session.id);
                ...
                <>
                  <div style={{ background: "var(--c2)", border: "1px solid var(--bd)", borderRadius: 10, padding: "14px 16px", marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <div>
                        <div style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: 28, color: "var(--gr)" }}>{total} <span style={{ fontSize: 13, color: "var(--t3)" }}>pt</span></div>
                        <div style={{ fontSize: 11, color: "var(--t3)", marginTop: 2 }}>
                          {myRank > 0 ? `#${myRank} van ${leaderboard.length}` : "Nog geen rang"}
                        </div>
                      </div>
                      <div style={{ textAlign: "right", display: "flex", flexDirection: "column", gap: 4 }}>
                        <span className="pts-badge pts-10">{exact}× exact (+{exact * 10})</span>
                        <span className="pts-badge pts-5">{near}× bijna</span>
                        {bp > 0 && <span className="pts-badge pts-7">+{bp} bonus</span>}
                        {sp > 0 && <span className="pts-badge" style={{ background: "rgba(59,130,246,.15)", color: "#60a5fa", border: "1px solid rgba(59,130,246,.25)" }}>+{sp} eindstand</span>}
                      </div>
                    </div>
                    <div style={{ height: 6, background: "var(--c3)", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ width: `${maxPts > 0 ? Math.round(total / maxPts * 100) : 0}%`, height: "100%", background: "var(--gr)", borderRadius: 3, transition: "width .8s ease" }} />
                    </div>
                    <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 4 }}>van {maxPts} (koploper)</div>
                  </div>
                  ...
                  {myPreds.length === 0
                    ...
                    : <div className="card">
                      {myPreds.map(pred => {
                        ...
                        let cls = "pts-0", lbl = "—";
                        if (done) {
                          const r = scorePts(pred.home_goals, pred.away_goals, m.home_goals, m.away_goals);
                          cls = r.cls; lbl = r.label;
                        }
                        return (
                  ...
                  return (
                    <div key={u.id} className="ur">
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: "var(--t3)", marginTop: 2 }}>{up.length} tips · {exact}× exact · {bp}pt bonus · {sp}pt stand</div>
                      </div>
                      <div style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: 18, color: "var(--gr)", marginRight: 8, flexShrink: 0 }}>{pts + bp + sp}<span style={{ fontSize: 10, color: "var(--t3)" }}> pt</span></div>
